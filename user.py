...
class User(object):
    ...
    def get_token(self):
        ...
        try:
            ...
            print('time out when getting token,retrying...{}'.format(self.__count))
            self.get_token()
    ...
    # if using seat,return True,stat(RESERVE,CHEKE_IN,AWAY),id ,loc, begin, end;
    # else return False,None,None,None.None
    def reservation(self):
        ...
        for r in reservations:
            if r.get('stat') == 'RESERVE' or r.get('stat') == 'CHECK_IN' or r.get('stat') == 'AWAY':
                return True, r.get('stat'), str(r.get('id')), r.get('loc'), r.get('begin'), r.get('end')

        return False, None, None, None, None, None

    # if using seat,return True,id ,loc, begin, end;
    # else return False,None,None,None,None
    def reservation_reserve_status(self):
        token = self.__token
        url = 'https://seat.lib.whu.edu.cn:8443/rest/v2/history/1/10'
        headers = {
            'token': token,
        ...
        try:
            ...
            self.get_token()
            self.count()
            return self.reservation_reserve_status()
        except  requests.RequestException:
            print('error when getting reservation :', requests.RequestException)
            return False, None, None, None, None
        reservations = response.json().get('data').get('reservations')
        for r in reservations:
            if r.get('stat') == 'RESERVE':
                print('there is a valid reservaton')
                return True,  str(r.get('id')), r.get('loc'), r.get('begin'), r.get('end')
    ...
    def stop_cancel(self):
        b, status, id, loc, begin, end = self.reservation()
        # 无正在使用的座位,return
        if b == False:
            return
        else:
